...
# 1. Native Multi-Format Text Extraction (Zero DLL Dependencies)
...
# 2. Resilient AI Generator targeting Gemini 3.x Flash
...
# 3. Clean Dark-Mode Graph Generator
...
# 4. Bot Onboarding Helper
async def send_welcome_guide(update: Update):
    msg = (
        "👋 <b>Welcome to MatchPro ATS Bot!</b>\n\n"
        "1️⃣ <b>Set Job Requirements:</b> Send <code>/jd &lt;Paste job description text&gt;</code>\n"
        ...
        "4️⃣ <b>Reset:</b> Send <code>/clear</code> to wipe current staged files."
    )
    await update.message.reply_text(msg, parse_mode=ParseMode.HTML)

# 5. Core ATS Evaluation Engine
...
{courses_text}"""
        ...
        # AUTOMATIC CLEANUP: Reset staged resumes so the next run starts fresh
...
async def set_jd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Store JD without deleting already uploaded resumes
...
# 10. Smart Conversational & Command Text Router
async def handle_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_msg = update.message.text.strip().lower()
    
    # 1. Benchmark Triggers
    if user_msg in ["compare", "analyze", "benchmark"]:
        await execute_evaluation(update, context)
        return
